...
const data = [
  { report: 'Not arrived', numOfIncidents: 53240 },
  { report: 'Damaged items', numOfIncidents: 96700 },
  { report: 'Ran out of stock', numOfIncidents: 118780 },
  { report: 'Low quality', numOfIncidents: 113870 },
  { report: 'Low quantity', numOfIncidents: 159620 },
  { report: 'Late arrival', numOfIncidents: 194479 },
];
    ...
    <div>
      <div>
        <Tooltip.Content
          ...
          text={data[targetItem.point].report}
        />
      </div>
      <div>
        <Tooltip.Content
...
const formatInfo = (target) => {
  ...
  const argument = data[point].report;
  return `${value} reports occurred of ${argument}`;
};
...
const decodeTarget = code => ({ series: code % 2 ? 'China' : 'numOfIncidents', point: Math.floor(code / 2) });

const compareTargets = (
  { series, point }, { series: targetSeries, point: targetPoint },
) => series === targetSeries && point === targetPoint;

class PieChartTooltip extends React.PureComponent {
  constructor(props) {
    super(props);

    this.state = {
      hover: null,
      selection: [{ series: 'numOfIncidents', point: 3 }],
      tooltipTarget: null,
      tooltipEnabled: true,
    };
...
export default withStyles(styles)(PieChartTooltip);
